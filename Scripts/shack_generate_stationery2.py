"""
SHACK ENTERTAINMENT — shack_generate_stationery2.py
Stationery family v2 — department marks on the right of the page.
Usage:
  py shack_generate_stationery2.py            equal marks
  py shack_generate_stationery2.py au|le|snn  that lane's mark enlarged
Logos live in Brand\\ as logo_shack/au/le/snn.png.
"""
import os, sys
...
def base():
    ...
    def para(text, size, bold=False, color=NAVY, after=0, where=None):
        p = (where or doc).add_paragraph()
        r = p.add_run(text)
    ...
    para('SHACK', 26, True, where=left)
    para('ENTERTAINMENT', 26, True, after=2, where=left)
